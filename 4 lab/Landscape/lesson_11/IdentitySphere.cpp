...
glm::vec3 GetPositionOnSphere(float u, float v, unsigned radius)
{
...
CIdentitySphere::CIdentitySphere(unsigned slices, unsigned stacks, unsigned radius)
	:m_radius(radius)
{
...
void CIdentitySphere::Tesselate(unsigned slices, unsigned stacks)
{
    ...
    for (unsigned ci = 0; ci < slices; ++ci)
    {
        ...
        for (unsigned ri = 0; ri < stacks; ++ri)
        {
            ...
            SVertexP3NT2 vertex;
            vertex.position = GetPositionOnSphere(u, v, m_radius);
