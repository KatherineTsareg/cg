...
CFlower::CFlower(float minRadius, float maxRadius, int petalCount)
...
void CFlower::SetPosition(const glm::fvec2 & position)
{
	m_position = position;
}
...
void CFlower::Redraw() const
{
    ...
    for (float angle = 0; angle <= float(2 * M_PI) + 0.5f * step; angle += step)
    {
        ...
        glVertex2f(m_position.x + x, m_position.y + y);
    }
